#[macro_export]
macro_rules! GET {
    ($func_name:ident, ($path:expr $(, $arg:ident:$T:ty)*), $RequestType:ty, $ResponseType:tt) => {
        pub fn $func_name(
            &self $( , $arg: $T)*, request: &$RequestType, access_token: Option<&AccessToken>
...
macro_rules! PATCH {
    ($func_name:ident, ($path:expr $(, $arg:ident:$T:ty)*), $RequestType:ty, $ResponseType:tt) => {
        pub fn $func_name(
            &self $( , $arg: $T)*, request: &$RequestType, access_token: &AccessToken
        ) -> Box<Future<Item = $ResponseType, Error = Error>> {
